...
void Attack_4_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void Attack_4_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_main_input_signal_no() {return 132;}

uint get_total_signal_no() {return 112896;}

uint get_number_of_components() {return 293;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 84112;}
...
ctx->componentMemory[coffset].componentName = componentName;
ctx->componentMemory[coffset].idFather = componentFather;
ctx->componentMemory[coffset].subcomponents = new uint[127];
}
...
Fr_lt(&expaux[0],&lvar[3],&circuitConstants[255]); // line circom 16
while(Fr_isTrue(&expaux[0])){
{
...
ctx->componentMemory[coffset].templateName = "Attack";
ctx->componentMemory[coffset].signalStart = soffset;
ctx->componentMemory[coffset].inputCounter = 132;
ctx->componentMemory[coffset].componentName = componentName;
ctx->componentMemory[coffset].idFather = componentFather;
ctx->componentMemory[coffset].subcomponents = new uint[83];
}
...
uint csoffset = mySignalStart+625;
for (uint i = 0; i < 1; i++) {
...
for (uint i = 0; i < 2; i++) {
std::string new_cmp_name = "attackChecker"+ctx->generate_position_array(aux_dimensions, 1, i);
mySubcomponents[aux_create+i] = aux_cmp_num;
IsEqual_3_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
csoffset += 6 ;
aux_cmp_num += 2;
...
Fr_lt(&expaux[0],&lvar[12],&circuitConstants[241]); // line circom 38
while(Fr_isTrue(&expaux[0])){
{
PFrElement aux_dest = &lvar[13];
// load src
Fr_idiv(&expaux[0],&lvar[12],&circuitConstants[7]); // line circom 39
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
uint cmp_index_ref = 0;
{
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + ((1 * (Fr_toInt(&lvar[12]) + 1)) + 1)];
// load src
Fr_mod(&expaux[0],&lvar[12],&circuitConstants[7]); // line circom 40
// end load src
Fr_copy(aux_dest,&signalValues[mySignalStart + (((3 * Fr_toInt(&lvar[13])) + (1 * Fr_toInt(&expaux[0]))) + 1)]);
}
...
PFrElement aux_dest = &lvar[12];
// load src
Fr_add(&expaux[0],&lvar[12],&circuitConstants[3]); // line circom 38
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_lt(&expaux[0],&lvar[12],&circuitConstants[241]); // line circom 38
...
Fr_eq(&expaux[0],&signalValues[mySignalStart + 132],&ctx->signalValues[ctx->componentMemory[mySubcomponents[0]].signalStart + 0]); // line circom 55
...
Fr_lt(&expaux[0],&lvar[13],&circuitConstants[81]); // line circom 59
while(Fr_isTrue(&expaux[0])){
{
uint cmp_index_ref = ((1 * Fr_toInt(&lvar[12])) + 1);
{
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + 1];
// load src
// end load src
...
{
uint cmp_index_ref = ((1 * (Fr_toInt(&lvar[12]) + 1)) + 1);
{
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + 1];
// load src
// end load src
...
Fr_mul(&expaux[1],&ctx->signalValues[ctx->componentMemory[mySubcomponents[((1 * Fr_toInt(&lvar[12])) + 1)]].signalStart + 0],&ctx->signalValues[ctx->componentMemory[mySubcomponents[((1 * (Fr_toInt(&lvar[12]) + 1)) + 1)]].signalStart + 0]); // line circom 68
...
PFrElement aux_dest = &lvar[12];
// load src
Fr_add(&expaux[0],&lvar[12],&circuitConstants[5]); // line circom 70
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
PFrElement aux_dest = &lvar[13];
// load src
Fr_add(&expaux[0],&lvar[13],&circuitConstants[3]); // line circom 59
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_lt(&expaux[0],&lvar[13],&circuitConstants[81]); // line circom 59
...
Fr_mul(&expaux[1],&ctx->signalValues[ctx->componentMemory[mySubcomponents[81]].signalStart + 0],&ctx->signalValues[ctx->componentMemory[mySubcomponents[82]].signalStart + 0]); // line circom 82
Fr_eq(&expaux[0],&expaux[1],&circuitConstants[3]); // line circom 82
...
Attack_4_create(1,0,ctx,"main",0);
Attack_4_run(0,ctx);
}
